...
| 7. DPA jadi "pagar" AI | ✅ selesai | **Backend SELESAI (PR #4)** + **UI mobile DPA (view+edit, chip editor, password confirm) — PR #<TBD>** |
...
**Berikutnya (credential-free, urutan saran):** ~~Customer JWT + QR identity backend (#4/#5)~~ ✅ **Day 4 (PR #7)** → ~~checkout endpoint (#6)~~ ✅ **Day 5 (PR #8)** → ~~analisis `top_product` (#8)~~ ✅ **Day 6 (PR #9)** → ~~UI mobile Login UMKM (#1)~~ ✅ **(PR #10)** → ~~UI mobile DPA (#7)~~ ✅ **(PR #<TBD>)**. Semua backend credential-free selesai. UI mobile (customer/QR) = track Flutter terpisah.
...
| Fitur MVP | Isi |
|---|---|
| ✅ **7. DPA jadi "pagar" AI** | Backend ✅ **selesai (PR #4)**: simpan DPA, cek deterministik Pre+Post, inject prompt. **UI mobile DPA SELESAI (PR #<TBD>)**: layar view+edit, chip editor, password confirm inline. |
