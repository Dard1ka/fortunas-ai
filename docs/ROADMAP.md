...
|---|---|---|
| 1. Login UMKM + HTTPS | 🟡 sebagian | Auth backend ✅ + **layar Login/Register Flutter ✅ (auth gate + token secure-storage + Profile akun)**. Belum: HTTPS/domain VPS |
...
**Berikutnya (credential-free, urutan saran):** ~~Customer JWT + QR identity backend (#4/#5)~~ ✅ **Day 4 (PR #7)** → ~~checkout endpoint (#6)~~ ✅ **Day 5 (PR #8)** → ~~analisis `top_product` (#8)~~ ✅ **Day 6 (PR #9)** → UI mobile Login UMKM (#1) ✅. Semua backend credential-free selesai. UI mobile (DPA #7-sisanya + customer/QR) = track Flutter terpisah.
