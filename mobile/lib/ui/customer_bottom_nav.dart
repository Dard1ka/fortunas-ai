...
/// Customer 5-slot bottom nav with the QR scan button raised in the center.
/// REQUIREMENTS §6.1: Beranda · Riwayat · Scan/QR (center) · Poin · Profil.
class CustomerBottomNav extends StatelessWidget {
  final String currentLocation;
  const CustomerBottomNav({super.key, required this.currentLocation});

  static const _items = <_NavSpec>[
    _NavSpec(label: 'Beranda', icon: 'home',    path: '/customer/home'),
    _NavSpec(label: 'Riwayat', icon: 'history', path: '/customer/history'),
    _NavSpec(label: 'QR Saya', icon: 'sparkle', path: '/customer/qr', primary: true),
    _NavSpec(label: 'Poin',    icon: 'coin',    path: '/customer/points'),
    _NavSpec(label: 'Profil',  icon: 'user',    path: '/customer/menu'),
  ];
    ...
    return Container(
      ...
      decoration: const BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [Color(0xFAFAF7F0), Color(0x00FAF7F0)],
          stops: [0.6, 1.0],
        ),
      ),
      child: Container(
        ...
        child: Row(
          ...
          children: [
            for (final item in _items)
              Expanded(
                child: item.primary
                    ? _QrFab(onTap: () => context.go(item.path))
                    : _NavItem(
                        item: item,
